...
matrix-multiply-accumulate (MMA), **warp schedulers** issuing instructions to groups of 32 lockstep
threads ("**warps**", SIMT), plus registers, shared memory/L1, L2, and HBM. The program model is
**Grid → Block → Warp → Thread**: the grid is launched, blocks are scheduled onto SMs (a block
never spans two SMs), and inside an SM every 32 threads form a warp driven by one scheduler. The
most important trick: **a warp stalling on HBM does not stall the SM** — the scheduler switches
to another ready warp; with enough warps resident, outstanding loads keep the HBM pipeline
saturated, so *slow* memory *behaves* like fast memory. Prefill (big dense GEMMs) lives on Tensor
Cores with high occupancy; decode (streaming weights, one token at a time) lives on latency
hiding — see [Rooﬄine](../Inference/Roofline.md) and
...
1. **Massive parallelism.** A `[S,d]×[d,d]` GEMM at S=8192, d=4096 is 4096 independent dot
   products of length 8192×4096 — embarrassingly parallel; attention adds S×S×h independent heads.
...
The flip side: a GPU is a *poor* general-purpose CPU (slow per-thread control flow). LLM
inference is ~90% GEMM/GEMV + elementwise ops (`GEMM.md`), so it lands in the GPU's sweet spot [I].
...
- **Tensor Cores** — mixed-precision **MMA** units: one instruction multiplies-accumulates small
  tiles (e.g. 16×8×16) in FP16/BF16/FP8/FP4; the entire "989 TFLOP BF16 dense" peak comes from
  here, not the CUDA cores [F: H100 spec]. See `Tensor-Cores.md`.
- **Warp schedulers** — 4 per SM [F: H100 whitepaper], each issuing one instruction per clock.
...
prediction. A **GPU**: hundreds of narrow, mostly in-order, throughput-optimized cores with far
more on-chip scratch and much wider memory bandwidth.
### Why
...
guide, SIMT model]. Neural nets are where "lots of threads" beats "few fast threads" by orders of
magnitude [I].
### How
CPU hides latency via **ILP** — speculative/out-of-order issue within a ~few-hundred-instruction
window. GPU hides latency via **TLP** — the warp scheduler swaps between ready warps, no
speculation. Both exist on a GPU; TLP is the dominant mechanism (Concept 4).
### When
...
### Hardware impact
A CPU die spends much of its area on cache + branch hardware [A]; a GPU die spends most of its
area on SMs. H100: 132 SMs [F] vs a server CPU's ~16–32 cores.
### Inference impact
...
- **PCIe fallback:** host RAM ↔ HBM at ~64 GB/s is ~50× slower than HBM's 3.35 TB/s for the same
  tensor [E: 3350/64 ≈ 52].
### How to measure it
`nvidia-smi --query-gpu=utilization.gpu` alongside host CPU %; Nsight Systems shows the gaps.
Oscillating GPU util tracking host CPU spikes = CPU-bound.

## Concept 2 — SIMT and the Warp
### What
Hardware groups every **32 consecutive threads of a block into a warp**, and the warp scheduler
executes **one instruction at a time for all 32 lanes in lockstep** — SIMT, single instruction
multiple threads. Warp size 32 is fixed hardware, not a launch parameter
...
**Divergence**: `if (lane < 16) A else B` → the warp runs A with lanes 0–15 active, then B with
16–31; both paths are issued → up to 2× issue slots for that region.
### When
Always — SIMT is the model; you can't opt out. You *can* avoid *paying* for it by aligning control
flow with warp structure: branch on `blockIdx` or `threadIdx.x / 32`, not `threadIdx.x % 3`
[A: kernel-writing convention].
### Hardware impact
The warp scheduler is the heart of the SM: 4 schedulers × 1 issue/clock [F]. The instruction mix
(memory vs ALU vs Tensor MMA) decides which SM sub-unit saturates — what "warps stalled on memory"
metrics measure.
### Inference impact
...
registers/thread, one SM holds at most 65,536/R threads = 65,536/(32·R) warps — Concept 4.
...
contend for the same 3.35 TB/s, so "more SMs" does not raise memory-bound throughput
(`Bandwidth-vs-Compute.md`).
### Inference impact
...
- **TLP** — many *warps* provide independent work (the GPU's dominant mechanism; bounded only by
  the register file and warp slots → where **occupancy** comes in).

### What
**Occupancy** = resident warps on an SM ÷ the SM's maximum (64 warps / 2048 threads on H100
[F: H100 spec]). It caps how much latency you can hide: more resident warps ⇒ more bytes in
flight ⇒ closer to peak HBM bandwidth.
### Why
...
covered by e.g. 64 resident warps/SM × ~1.5 loads each. So even *modest* per-SM occupancy
saturates bandwidth; 100% occupancy is sufficient, not necessary [I].
### How
...
- **64 regs/thread**: 65,536/64 = 1,024 threads = **32 warps = 50% occupancy** [E]. Halving
  ...
  memory/block, blocks-per-SM; the compiler reports which resource binds.)
### When
...
- **Decode: this is where hiding lives.** Per SM the active work is thin (small M); the engine's
  batch is the TLP source. B=64 gives 64× the warps of B=1 for the same weights — the same 3.35
  TB/s now feeds 64 sequences. That is the roofline's knee B* ≈ ridge·b_w/2 ≈ 295·2/2 ≈ **295
  (BF16, H100)** [E; `../Inference/Roofline.md`].
### Hardware impact
...
useful warps per instruction stream → achieved BW drops to a fraction of peak; that gap is ITL.
### Failure modes
...
**~14×** gap [E: 900/64] — why TP lives intra-node on NVLink while PP/EP/CP move to RDMA across
nodes (`Scale-Up-vs-Scale-Out.md`, `Topology.md`).
### How
...
model load is PCIe-bound, not CPU-bound. Same tensor from HBM: 54.0 GB ÷ 3.35 TB/s ≈ **16 ms** [E].
### Failure modes
