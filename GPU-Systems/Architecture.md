...
constants. Tags: **[F]** verified primary source · **[A]** engineering assumption · **[I]** inference · **[E]** verified by computation this session.
...
matrix-multiply-accumulate (MMA), **warp schedulers** issuing to groups of 32 lockstep threads
("**warps**", SIMT), plus registers, shared memory/L1, L2, and HBM. The program model is
**Grid → Block → Warp → Thread**: the grid is launched, blocks are scheduled onto SMs (a block
never spans two SMs); inside an SM, every 32 threads form a warp driven by one scheduler. The
most important trick: **a warp stalling on HBM does not stall the SM** — the scheduler switches
to another ready warp; enough resident warps keep the HBM pipeline saturated, so *slow* memory
*behaves* like fast. Prefill (big dense GEMMs) lives on Tensor Cores with high occupancy; decode
(streaming weights, one token at a time) lives on latency hiding — see [Roofline](../Inference/Roofline.md)
and [The Life of a Token](../Inference/The-Life-of-a-Token.md).
...
1. **Massive parallelism.** A `[S,d]×[d,d]` GEMM at S=8192, d=4096 is 4096 independent dot
   products — embarrassingly parallel; attention adds S×S×h independent heads.
2. **Latency-vs-throughput.** We never care about *one* element's latency, only the whole
   ...
   throughput-oriented GPU hides it with thousands of independent chains at once.
Flip side: a GPU is a *poor* general-purpose CPU — but LLM inference is ~90% GEMM/GEMV +
elementwise ops (`GEMM.md`), so it lands in the GPU's sweet spot [I].
...
- **CUDA cores** — scalar FP32/BF16/FP16/INT32 ALUs, one element per clock
...
- **Tensor Cores** — mixed-precision **MMA** units: one instruction multiplies-accumulates
  small tiles (e.g. 16×8×16) in FP16/BF16/FP8/FP4; the "989 TFLOP BF16 dense" peak comes from
  here, not the CUDA cores [F: H100 spec]. See `Tensor-Cores.md`.
- **Warp schedulers** — 4 per SM, one instruction issued per clock [F: H100 whitepaper].
- **Register file** — 256 KB = **65,536 × 32-bit registers** [F: H100 spec]; biggest occupancy lever.
- **Shared memory / L1** — per-SM SRAM, programmable split per launch [F: CUDA programming guide].
- **L2 cache** — one, whole-die shared, in front of HBM.
- **HBM** — main memory (HBM3 on H100 @ 3.35 TB/s [F: spec]; HBM3e on H200/B200; GDDR7 on RTX 50);
  bandwidth is device-wide, not per-SM.
...
prediction. A **GPU**: hundreds of narrow, mostly in-order, throughput-optimized cores, far more
on-chip scratch, much wider memory bandwidth.
### Why
...
guide, SIMT model] — which beats "few fast threads" by orders of magnitude on neural nets [I].
### How
CPU hides latency via **ILP** — speculative/out-of-order issue within a ~few-hundred-instruction
window. GPU hides latency via **TLP** — the warp scheduler swaps between ready warps; TLP is the
dominant mechanism (Concept 4).
### When
...
### Hardware impact
A CPU die spends much of its area on cache + branch hardware [A]; a GPU die on SMs. H100:
132 SMs [F] vs a server CPU's ~16–32 cores.
### Inference impact
All LLM math is GPU-resident; the CPU is only the *orchestrator*. Host-side stalls (Python, GIL,
in-loop sampling) add directly to ITL — a slow scheduler shows up as GPU idle time (`Diagnostics.md`).
### Example [E]
One 4096×4096 GEMM = 2·4096³ ≈ **137 GFLOP**. 16-core server CPU, AVX-512, no AMX-class BF16
speedup, ~27 GFLOP/s: 137e9/27e9 ≈ **5.1 s**. H100 at a conservative 30% of its 989 TFLOP BF16
peak (≈297 TFLOP/s): 137e9/297e12 ≈ **0.46 ms** — ~4 orders of magnitude on one layer's op.
### Failure modes
- **CPU-bound serving:** host can't feed the GPU → utilization dips between kernels (the classic small-batch decode symptom).
- **PCIe fallback:** host RAM ↔ HBM at ~64 GB/s is ~50× slower than HBM's 3.35 TB/s [E: 3350/64 ≈ 52].
### How to measure it
`nvidia-smi --query-gpu=utilization.gpu` alongside host CPU %; Nsight Systems shows the gaps; oscillating GPU util tracking host CPU spikes = CPU-bound.

## Concept 2 — SIMT and the Warp
### What
Hardware groups every **32 consecutive threads of a block into a warp**; the warp scheduler
executes **one instruction at a time for all 32 lanes in lockstep** — SIMT, single instruction
multiple threads. Warp size 32 is fixed hardware, not a launch parameter [F: CUDA programming
guide].
### Why
...
no mispredict, no pipeline flush; you just pay issue slots for the masked path [F: CUDA
programming guide].
### How
All 32 lanes execute `fma(r0,a,b,c)` in the same clock — "32 CUDA cores, one FMA, one cycle."
**Divergence**: `if (lane < 16) A else B` → the warp runs A (lanes 0–15 active), then B
(lanes 16–31); both paths are issued → up to 2× issue slots for that region.
### When
Always — SIMT is the model; you can't opt out. You *can* avoid *paying* for it: align control
flow with warp structure — branch on `blockIdx` or `threadIdx.x / 32`, not `threadIdx.x % 3` [A].
### Hardware impact
The warp scheduler is the heart of the SM: 4 schedulers × 1 issue/clock [F]. The instruction mix
(memory/ALU/MMA) decides which SM sub-unit saturates — what "warps stalled on memory" measures.
### Inference impact
...
- **Decode** GEMVs spread work over few threads per output; engines keep control flow
  warp-aligned, so the decode cost is *latency* (stalls), not divergence.
### Example [E]
Grid of **8 blocks × 256 threads = 2048 threads = 64 warps** (2048/32); each block forms 8
warps. If an SM hosts 2 blocks (16 warps) and each warp issues one 32-wide FMA/cycle:
16 FMA/cycle = 16×32 = **512 FLOP/cycle from scalar ALUs alone** — before any Tensor Core work.
### Failure modes
...
- **Assuming independent threads:** two lanes of a warp *must* run the same instruction stream; per-thread control flow pays the mask.
- **Divergent memory access:** two lanes reading distant addresses split one warp load into multiple transactions (`Memory-Hierarchy.md`).
### How to measure it
Nsight Compute: branch-instruction and warp-branch-divergence reports; issue-slot utilization (warps issued / warps eligible).
...
```
- **Thread** — `threadIdx` + registers; that's all it privately owns. **Warp (32)** — unit of execution; scheduler granularity.
...
Occupancy ties the levels together: 65,536-register file [F] ÷ R regs/thread = 65,536/R threads
= 65,536/(32·R) warps per SM — Concept 4.
...
engineering = moving each byte **as few levels down as possible**, never stranding a warp at
the bottom.
### Why
HBM latency (~hundreds of ns) is structurally ~30–100× an L1 hit [A]; no caching makes HBM fast,
so the GPU makes latency *invisible* instead (Concept 4).
### How
HBM → (coalesced warp-wide load) → L1/L2 → shared memory → registers → ALU/Tensor Core. Tiling
turns O(N·K) HBM reads/output into O(N·K/BM): load a 64×64 tile once, reuse it 64× (`GEMM.md`).
### When
Registers: GEMM inner loop, activation math. Shared: GEMM/attention tiles, reduction partials.
L2: KV re-reads that fit, weight prefetch. HBM: everything else — at decode, *all* weights + *all* KV, every token.
### Hardware impact
Shared memory is per-SM: a block on SM7 cannot read SM3's. HBM bandwidth is device-wide — all SMs
contend for the same 3.35 TB/s, so "more SMs" does not raise memory-bound throughput (`Bandwidth-vs-Compute.md`).
### Inference impact
Prefill attention's S×S score matrix at S=32k, h=64 = 32768²×64×2 B ≈ **137 GB = 128 GiB** [E]
must never reach HBM; FlashAttention keeps it in shared memory [F: arXiv:2205.14135]. Decode streams
the whole weight matrix from HBM each token; bytes/token is the ITL denominator (`../Inference/The-Life-of-a-Token.md`).
### Example [E]
...
### How to measure it
Nsight Compute memory table: `dram__throughput` vs `l1tex__throughput`, sector hit rates; bandwidth-bound kernel → push `dram__throughput` toward the 3.35 TB/s roof.
...
bandwidth approaches peak** — memory *looks* fast even though each load still takes ~500 ns [I:
standard throughput-architecture argument; latency values are engineering estimates].
Two distinct hiding mechanisms: **ILP** — one warp has multiple independent instructions in
flight (CPU-style, compiler-window-bounded); **TLP** — many *warps* provide independent work (the
dominant mechanism; bounded only by the register file and warp slots → where **occupancy** comes
in).

### What
**Occupancy** = resident warps on an SM ÷ the SM's maximum (64 warps / 2048 threads on H100
[F: H100 spec]) — it caps how much latency you can hide: more warps ⇒ more bytes in flight ⇒
closer to peak HBM bandwidth.
### Why
...
(1.67 MB / 128 B) [E] = ~**99 outstanding loads per SM** over 132 SMs [E] — covered by 64 resident
warps/SM × ~1.5 loads each. So *modest* per-SM occupancy saturates bandwidth; 100% is sufficient, not necessary [I].
### How
Register-file arithmetic (H100: 65,536 regs/SM [F], cap 64 warps/SM [F]):
- **32 regs/thread**: 65,536/32 = 2,048 threads = **64 warps = 100%** of the 64-warp cap [E].
- **64 regs/thread**: 65,536/64 = 1,024 threads = **32 warps = 50% occupancy** [E]. Halving
  registers/thread doubles occupancy — the core GEMM/attention tuning knob. (Other caps: shared
  memory/block, blocks-per-SM; the compiler reports the binding resource.)
### When
...
- **Decode: this is where hiding lives.** Per SM the active work is thin (small M); the
  engine's batch is the TLP source. B=64 gives 64× the warps of B=1 for the same weights — the
  same 3.35 TB/s now feeds 64 sequences. That is the roofline's knee B* ≈ ridge·b_w/2 ≈ 295·2/2 ≈
  **295 (BF16, H100)** [E; `../Inference/Roofline.md`].
### Hardware impact
Register file + warp slots + shared memory are the three occupancy caps. Tensor Cores amplify
occupancy: one warp's MMA covers 16×8×16 elements, so GEMM can be FLOP-saturated at ~50% occupancy
while a copy kernel is bandwidth-starved at 50% [I].
### Inference impact
- **Decode B=1:** few active warps per SM → HBM not fully utilized → measured tok/s well below
  `BW/bytes`. Continuous batching (`../Inference/Continuous-Batching.md`) is literally adding
  resident warps to fill the memory pipeline [I].
...
useful warps → achieved BW drops to a fraction of peak; that gap is ITL.
### Failure modes
- **Low occupancy + memory-bound kernel:** #1 decode inefficiency; check `occupancy` + warp-stall
  breakdown in Nsight Compute.
- **Register spills:** chasing occupancy with few regs spills to local memory (HBM) — bandwidth collapses even when the metric says "high occupancy".
...
- Rule: memory-bound + `long_scoreboard` dominant + occupancy < 50% → add warps (bigger batch, fewer regs/thread). Nsight Systems: kernel durations + `dram__throughput` vs roof.
...
### Why
Multi-GPU inference is a fabric problem: TP does 2 AllRecommends per layer, so per-layer latency
adds `2×(link latency + comm time)`. NVLink ~900 GB/s vs PCIe ~64 GB/s is a **~14×** gap [E: 900/64]
— why TP lives intra-node on NVLink while PP/EP/CP move to RDMA across nodes (`Scale-Up-vs-Scale-Out.md`, `Topology.md`).
### How
PCIe: driver posts DMA; `cudaMemcpy` moves ~64 GB/s. NVLink: direct GPU↔GPU, bypassing the host;
NCCL's AllReduce/AllGather and P2P copies run over it. NVSwitch: an on-node switch chip wiring all
8 GPUs' NVLinks into a full crossbar — an HGX node becomes one machine instead of 8 PCIe islands [I].
### When
PCIe: single-GPU boxes, dev rigs, model loading. NVLink: any TP, in-node P/D KV, intra-node MoE
AllToAll. NVSwitch/NVL72: 72-GPU domains for frontier MoE (DeepSeek-class) EP/CP [F: NVIDIA NVL72].
### Hardware impact
Fabric bandwidth bounds communication-parallelism: AllReduce time scales with payload/BW. NVSwitch
makes every AllReduce uniform, while PCIe-only topologies produce near/far pairs where NCCL routes
through the host — ~14× slower on that path [I].
### Inference impact
- **TP=8 on NVLink:** 32 MB allreduce ≈ 2×32 MB/900 GB/s ≈ **0.07 ms** [E] → ~2 ms over 32
  layers ≈ <0.1% of a decode step; same on PCIe ≈ 1 ms/layer → ~32 ms → ~50% of a 65 ms ITL step [E]. The fabric *is* the parallelism strategy.
- **P/D disaggregation:** prefill→decode KV transfer is bulk copy. 1 GiB at NVLink 900 GB/s ≈
  **1.2 ms** [E]; at PCIe 64 GB/s ≈ **17 ms** [E] — a full ITL by itself (`Prefill-Decode-Disaggregation.md`).
### Example [E]
27B model (50.3 GiB BF16) loading onto one H100 over PCIe 5.0: 54.0 GB ÷ 64 GB/s ≈ **0.85 s** —
model load is PCIe-bound, not CPU-bound; the same tensor from HBM: 54.0 GB ÷ 3.35 TB/s ≈ **16 ms** [E].
### Failure modes
- **PCIe-only node running TP=8:** collective time ≈ compute time → "utilization high but
...
### How to measure it
`nccl-tests` (all_reduce_perf, alltoall_perf) on the *real* topology — never trust datasheet
BW; `nvidia-smi topo -m` for actual GPU↔GPU links; Nsight Systems + NCCL debug logs; NVLink throughput counters vs peak.
...
**warp/occupancy arithmetic decides whether you get it** (Concept 4) → **fabric decides**
(Concept 5). Kernel-level detail: `CUDA-From-Zero.md`, `Kernel-Life.md`; `Profiling.md`,
`GPU-Metrics.md`.
...
3. "Why does slow memory look fast?" → **TLP via warp switching**: enough resident warps keep
   HBM's ~1.67 MB in-flight window full, so achieved BW ≈ peak; occupancy is that arithmetic.
...
## References
NVIDIA CUDA C++ Programming Guide (warp = 32, SIMT, block/SM mapping, shared mem + L1 split)
[F] · NVIDIA H100 Datasheet/whitepaper (132 SMs, 989 TFLOP BF16 dense, 3.35 TB/s HBM3, 65,536
regs/SM, 64 warps/SM, ~900 GB/s NVLink, 4 warp schedulers/SM) [F: vendor spec] · NVIDIA
H200/B200/NVL72 announcements (HBM3e 141 GB, ~8 TB/s HBM3e, FP4/FP8 rates, 72-GPU NVLink domain)
[F: vendor spec] · Dao et al. 2022, FlashAttention [F: arXiv:2205.14135] ·
