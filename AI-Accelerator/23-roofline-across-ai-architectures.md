...
The *roofline model* (Samuel Williams, 2009, for the *GPU*) says: *a kernel's* *achieved* *rate* is *capped by* *the* *minimum* *of* *two* *roofs*:
```
...
*The* *six* *chips'* *rooflines* (at *the* *stated* *precision*, *dense*, *per* chip):
...
| Cerebras WSE-2 (FP16) | ~750 TFLOPS | on-wafer SRAM (not HBM; 21 PB/s aggregate [F: p12]) | n/a (SRAM regime) | **no ridge — SRAM regime** [I] |
| Groq TSP (INT8) | ~737 TOPS | on-chip streaming 20 TiB/s aggregate [F: p14] | n/a (SRAM regime) | **no ridge — SRAM regime** [I] |
| AWS Trainium2 (cFP8) | 158 TFLOPS | 2.9 TB/s | 2.9e12 × I | [E] ≈ **54** |

*The* *first-principles* *read:* **the* *ridge point is* *the* *chip's* *batch-1 threshold.* *A* *batch-1 GEMV* (decode) *has* *intensity* *~1* (page* 22) *— far* *below* *every* *HBM* *chip's* *ridge* (54–591) *— so* *batch-1 decode* is *bandwidth-bound* *on* *every* *HBM* chip. *That* *one* *number* *is* *the* *entire* *memory* *wall* *argument* (page* 03) *made* *quantitative.

## Placing the two LLM phases
*Using* *page 22's* *intensities* for *Llama-2 70B:
- **Prefill** (*intensity* ~4,100 [E]): *above* *the* *ridge* of *every* *HBM chip* → *compute-bound*. The *achieved* rate *is* *the* *peak* (at *~100% efficiency). *The* *FLOP* ceiling *is* the *limit.
- **Batch-1 decode** (*intensity* ~1 [E]): *below* *the* *ridge* of *every* *HBM chip* → *bandwidth-bound. The* *achieved* rate *is* *bandwidth × 1* (at *~100% efficiency), *i.e.,* *the* *HBM bandwidth* *in* *tokens/s* (page* 22's *formula). *The* *peak* *roof* *is* *irrelevant* (the *compute* *sits idle.
...
4. **Change the precision** (raise both roofs at once): *FP16→FP8* *doubles* *the* *peak* *and halves* the *bytes-per-param* (the *slope's* *denominator), *so* *both* *roofs* *move* in *the* *right* *direction* (page* 20). *Cost:* *accuracy (recovered by *microscaling.
...
- *Prefill* (intensity 4,100 [E]): *4,100 > 295* → *compute-bound.* Achieved ≈ *989 TFLOPS × efficiency.* At *~90%* GEMM efficiency [I]: [E] ~890 TFLOPS. A *4,096-token* prompt (555 TFLOP [E]: page 22) takes [E] 555e12 / 890e12 ≈ **~0.63 s** *on one H100* (in practice sharded across 8, faster).
- *Batch-1 decode* (intensity ~1 [E]): *1 < 295* → *bandwidth-bound.* Achieved ≈ 3.35e12 FLOP/s × 1 (per chip, at 100% bandwidth efficiency). The *token rate* is [E] 3.35e12 / 16.95e9 (the ⅛-shard, page 15) ≈ **198 tok/s at 100% efficiency**, ~60–100 at 30–50% [I].

*The* *two* *phases* *on the* *same* *roofline*: *prefill* *touches* the *peak roof* (the *989* *line); *decode* *touches* the *slope* (the *3.35* *line at* *intensity 1). *The* *chip* *is* *used* *at* *~90%* *of* its *compute* in *prefill* and *at* *~3–5%* *of* its *compute* in *decode* (the *bandwidth* *is* *the* *limit, *the* *compute* *is* *idle). *That* *asymmetry* *is* *the* *reason* *for* *disaggregated prefill/decode systems* [I].
