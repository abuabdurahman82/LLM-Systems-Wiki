...
- **The cost:** *the model must fit*, and *the scale-up domain* must be *big enough*. A 70B model at FP16 (137.95 GB [E]) *does not fit* in one Groq TSP (220 MiB [F]) — it must be *spread* across *576 TSPs* (page 14). The *scheduled Dragonfly* is what makes that *576-chip spread* *feel like one machine*.
...
**The model footprint (Llama-2 70B, 68.98 B params [F: HF checkpoint index]):**
- FP16: [E] 68.98 × 2 = **137.95 GB** of weights.
- INT8: [E] 68.98 × 1 = **68.98 GB** of weights.
- FP8: [E] 68.98 × 1 = **68.98 GB** of weights (same bytes, different numerics).
...
| TPU v4-4 (HBM) | 4 × 32 GiB = 128 GiB | *barely* (137.95 GB > 128 GiB, needs sharding across 4) | yes |
| Trainium2-8 (HBM) | 8 × 96 GiB = 768 GiB | yes | yes |
| Groq 576-TSP (SRAM) | [E] 576 × 220 MiB = **123.75 GiB ≈ 132.5 GB** | **no** (137.95 GB > 132.5 GB) | **yes** (68.98 GB, with 63.5 GB headroom) |
...
- A 70B model at FP16 (137.95 GB, sharded across 8 → 17.24 GB/chip): [E] 3.35e12 / 17.24e9 ≈ **194 tok/s** per chip at 100% efficiency (page 15's worked example).
...
- Per token: [E] 2 × 80 × 8 × 128 × 2 bytes = **327,680 bytes = 320 KiB/token**.
- At C = 4,096 tokens: [E] 320 KiB × 4,096 = **1.25 GiB (≈ 1.34 GB)** of KV cache *per request*.

For the Groq 576-TSP system (132.5 GB aggregate SRAM, 68.98 GB INT8 weights):
- Headroom: [E] 132.5 − 68.98 = **63.5 GB** for KV + activations.
- Max batch-1 context: [E] 63.5 GB / 320 KiB ≈ **194,000 tokens** — so the *KV cache* is *not* the constraint at C = 4,096; the *weights* are [I].

For an H100-8 (640 GB HBM, 137.95 GB FP16 weights):
- Headroom: [E] 640 − 137.95 = **502.1 GB** for KV + activations.
- Max batch-1 context: [E] 502.1 GB / 320 KiB ≈ **1.5 million tokens** — the *KV cache* is *not* the constraint here either; the *HBM bandwidth* is [I].
