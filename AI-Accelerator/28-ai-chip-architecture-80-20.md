...
The *field* *is* *large* (six *flagships,* *three* *regimes,* *five* *scheduling* *styles,* *five* *precision* *formats,* *two* *memory* *philosophies,* *three* *interconnect* *topologies,* *four* *software* *stacks, *plus* *the* *tail). *But* *almost* *all* *of* *it* *collapses* *into* *five* *first-principles* *facts:*
...
|---|---|---|
| 1 (matmul + memory) | you can *estimate* the FLOPs and bytes of any workload before you buy (page 22: Llama-2-70B prefill P=4,096, B=1 → 565 TFLOP, 137.95 GB weights [E]) | the vendor's "optimized for LLMs" claim |
...
3. *Does the model fit on-chip? *→ Fact 1+3 (page 17: Llama-2-70B-INT8 = 68.98 GB [E] → 1 HBM chip, 576 TSPs [F: NP 2023]).
...
15. *What* *is the* *power* *per* *rack? *→ Fact 3 (page 24: 3.3 kW box vs ~41 kW DGX-H100 rack).
