...
> **Status:** LIVING · **LAST_UPDATED:** 2026-08-31 · **SOURCE_DATE:** 2026-08-16 · **RESEARCH_STATUS:** initial build complete; current-coverage sections verified against live sources on 2026-08-16
>
> How it reads: `MATH → TRANSFORMERS → TRAINING → POST-TRAINING → DISTILLATION → REASONING → INFERENCE → SPECULATIVE DECODING → TRITON → OPTIMIZATION → DISTRIBUTED SYSTEMS → AGENTS → HARNESS ENGINEERING → CURRENT RESEARCH`

---

## What's new
- **2026-08-31 — homepage chrome:** `Distillation/`, `Speculative-Decoding/`, and first-class `Triton/` are now in the layout tree, sidebar, and reading guides (content was already on disk).
- **2026-08-27 — `Triton/` section (first-class starter):** GPU kernel language & compiler handbook — not NVIDIA Triton Inference Server. Landing + `01-what-is-triton.md` + `FACTS.md` (GB10 / Triton 3.6.0 measurements). Outline for pages 02–20 lives on the section README; `GPU-Systems/Triton.md` stays the single-page overview.
- **2026-08-27 — `Speculative-Decoding/` handbook expansion:** draft-verify → taxonomy → classical / retrieval / SpecInfer (pages 01–07 on disk; 08–21 outlined on the section README). Five-generation lens through EAGLE, MTP, DSpark.
- **2026-08-27 — `Distillation/` section (21 pages):** LLM Knowledge Distillation — Compressing Frontier Intelligence into Smaller Models, the full zero-to-hero arc from "what does distillation mean" to designing/training/deploying/benchmarking a distilled LLM. Pages: README (reading path + one-page mental model) · 01 why (cost problem, efficiency landscape, capacity gap) · 02 classical KD (history 2006→2026 timeline with primary-source-verified dates, soft targets, temperature, dark knowledge) · 03 losses (forward vs reverse KL mode-covering/seeking, JS/skew, MiniLLM) · 04 taxonomy (17-branch tree, black/white-box split, per-technique tables, decision tree) · 05 logit/feature/attention KD ([E] storage math: full logits 0.29 MiB/token → 375–500× top-K saving; cross-tokenizer; MoE→dense) · 06 sequence/response KD (the honest "synthetic data + SFT" framing) · 07 reasoning distillation (answer-vs-trace, data-quality pathologies, verified pipelines, best-of-N, active/curriculum) · 08 **DeepSeek-R1 case study** (every number verified against arXiv:2501.12948: 800K samples ≈600K reasoning + 200K non-reasoning, SFT-only, six students with per-model LRs, Table-15/16 benchmarks incl. distill-beats-RL at 32B) · 09 self & multi-teacher · 10 on-policy distillation (exposure bias; the 2025–26 turn: per-token reverse-KL as dense reward, 9–30× cheaper than RL [F: GKD arXiv:2306.13649 + Thinking Machines 2025]) · 11 GKD deep dive (TRL GKDTrainer) · 12 KD↔RL (preference & reward-model distillation) · 13 agentic/tool-use distillation · 14 data pipelines (cleaning, contamination workflow, what-degrades-first, hallucination/calibration/safety, lineage, 12 failure modes, myths-vs-reality) · 15 systems ([E] weight-memory tables 1.5B–671B × BF16→NVFP4; online vs offline teacher; distributed logit transfer; API-vs-open-weight legal discipline; enterprise/sovereign patterns) · 16 vs quantization/pruning/speculative decoding (distilled drafters; the 70B→7B→NVFP4 stack) · 17 benchmarking (retention/compression ratios, capability-per-dollar, Pareto, break-even N* = C_distill/(C_teacher−C_student)) · 18 labs (home 5090/4090, 2×DGX Spark, RTX PRO 6000; error mining; 6-phase iteration) · 19 production (offline + on-policy architectures, framework support matrix incl. TRL GKDTrainer, lineage template) · 20 future/glossary/references. Non-duplicating by cross-link: SFT/RLHF/DPO/GRPO stays in `Post-Training/`, test-time compute in `Reasoning/`, precision in `Quantization/`, draft-verify in `Speculative-Decoding/`. All [E] arithmetic machine-verified this session; all primary sources (arXiv IDs + Thinking Machines blog + TRL docs) fetched live 2026-08-27.
...
├── Distillation/              ← FIRST-CLASS (2026-08-27): LLM Knowledge Distillation — classical KD → taxonomy → reasoning distillation → R1 case study → on-policy/GKD → data pipelines → systems → benchmarking → labs → production; 21 pages, primary-source-verified
...
├── Speculative-Decoding/      ← FIRST-CLASS (2026-08-27): draft-verify handbook — why / algorithm / acceptance / taxonomy / classical / retrieval / SpecInfer (01–07 on disk; 08–21 outlined: Medusa, EAGLE, MTP, DSpark, serving)
...
├── Triton/                    ← FIRST-CLASS (2026-08-27): Triton kernel language & compiler (not NVIDIA Triton Inference Server) — landing + 01 what-is-triton + FACTS; pages 02–20 outlined; GPU-Systems/Triton.md remains the overview
...
- **Inference engineer:** `Serving-Engines/Engine-Landscape.md` (the layer stack: engines vs packaging vs platforms) → `Inference/The-Life-of-a-Token.md` → `Inference/Roofline.md` → `Inference/Inference-Optimization.md` (what to apply FIRST, measured) → `Attention/` → `KV-Cache/` → `Serving-Engines/` (the big three's deep dives live in `GPU-Systems/`) → `Serving-Engines/Engine-Mega-Comparison.md` (7-way matrix + decision guide) → `Distributed-Inference/Overview.md` (cluster jobs + KV transfer) → `Distributed-Inference/NVIDIA-Dynamo.md` + `llm-d.md` + `Dynamo-vs-llm-d.md` → `Quantization/` → `Speculative-Decoding/` (01–07 handbook).
- **Distillation / student-model engineer:** `Distillation/README.md` → `01-why-distillation.md` → `02-classical-knowledge-distillation.md` → `08-deepseek-r1-distillation.md` → `10-on-policy-distillation.md` → `11-gkd.md` → `19-production-design.md`.
- **GPU kernel / Triton engineer:** `Triton/README.md` → `Triton/01-what-is-triton.md` → `GPU-Systems/Triton.md` (overview) → `GPU-Systems/CUDA-From-Zero.md` → `GPU-Systems/Kernel-Stack.md` → `Inference/Roofline.md`.
