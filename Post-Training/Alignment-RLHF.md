...
|---|---|---|---|
| 2022 | **InstructGPT** (Ouyang et al., arXiv:2203.02155 [F]) | first full SFT→RM→PPO-RLHF pipeline; smaller aligned GPT-3 preferred over *unaligned* larger GPT-3 | 3-model cost (policy + RM + ref); PPO instability; reward-hacking |
...
| 2024 | **OpenAI o1 / DeepSeek-R1** | **RLVR**: reward = verifiable correctness (math/code/unit tests), no human preferences | only where answers are checkable; reward-hacking on verifiers; cost of thinking tokens |
...
wrote* — the ceiling is the demonstration quality, and the optimum is the *marginal*, not
the *mode* (mode collapse toward average answers [I]).

### Stage 2 (classic RLHF): reward model + PPO
1. **RM:** Bradley-Terry over preference pairs: `L_RM = -E[ log σ( r_θ(x, y_w) - r_θ(x, y_l) ) ]`.
2. **RL:** maximize `E[r_θ(x,y)] - β·KL(π_θ ‖ π_ref)` via PPO (arXiv:1707.06347; OpenAI
   implementation — GPT-4-era RLHF used PPO [F: InstructGPT report]).
...
- **Wins:** one model in memory, stable SGD dynamics, ~equal quality at scale
  [F: arXiv:2305.18290; Zephyr / Llama-2-Chat follow-ups].
...
- **Outcome RL (OPO/GRPO):** sample G completions per prompt; advantage =
  reward - group mean; no value model. DeepSeekMath introduced GRPO (arXiv:2402.03300
...
- **Why it works where RLHF stalled:** reward is *objective* and *dense* (a test suite
  gives 1000-bit signal vs a human's 1-bit preference), so the proxy-to-true gap that
  drove overoptimization mostly disappears — at the cost of only working where
  verification is cheap.

## Limitations (what's still unresolved)
- **Reward hacking / Goodhart:** any proxy reward gets exploited; KL anchors only slow
  it, they don't stop it [F: Gao 2022; Gold 2024].
...
- **Default pipeline:** SFT → DPO (or iterative DPO) → optional RLVR for reasoning tier.
  PPO-only pipelines are now rare outside frontier labs [I].
...
2. Classic RLHF = SFT + RM + PPO (3 models); DPO collapsed that to 1; RLVR collapsed
   *preferences* to *verifiers* where possible.
...
## References (verified 2026-08-18 via arXiv title-search)
...
- Gao et al. 2023 — Scaling Laws for RM Overoptimization. arXiv:2210.10760 [F]
- Lightman et al. 2023 — Let's Verify Step by Step. arXiv:2305.20050 [F]
- Wang et al. 2023 — Self-Consistency. arXiv:2203.11171 [F]
